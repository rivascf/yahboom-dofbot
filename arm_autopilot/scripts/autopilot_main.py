...
class LineDetect:
    def __init__(self):
        ...
        self.linear = 0.25 # 速度太快会导致机器人在移动的过程中看不到障碍物
        self.PID_init()
    ...
    def Wrecker(self, point_x, point_y):
        ...
        if abs(point_x - 320) < 10: point_x = 320
        if abs(point_y - 400) < 10: point_y = 400
        if abs(point_x - 320) < 10 and abs(point_y - 400) < 10:
            sleep(0.3)
            if self.ros_ctrl.RobotRun_status == True: self.ros_ctrl.pubVel(0, 0)
            ...
            joints = [pos1, 2.0, 60.0, 40.0, 90]
            # joints = [pos1, 8.0, 49.0, 46.0, 90]
            # print ("joints: ", joints, type(joints))
            if len(joints) != 0: self.arm_gripper(joints)
            self.color.msg_box = {}
    ...
    def arm_gripper(self, joints):
        joints.append(30)
        self.ros_ctrl.pubArm(joints, run_time=2000)
        sleep(2)
        self.ros_ctrl.pubArm([], id=6, angle=140)
        sleep(0.5)
